...
import datasets
from pre_functions import train, validate, LinearLrDecay, load_params, copy_params, cur_stages
    ...
    for epoch in tqdm(range(int(start_epoch), int(args.max_epoch)), desc='total progress'):
        ...
        if gen_net.module.cur_stage != cur_stage:
            ...
            dis_optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, dis_net.parameters()),
                                    args.d_lr, (args.beta1, args.beta2))
            # set up data_loader
        dataset = datasets.ImageDataset(args, cur_img_size=4*(2**(cur_stage+1)), bs=args.dis_batch_size//(2**(cur_stage)))
        train_loader = dataset.train
